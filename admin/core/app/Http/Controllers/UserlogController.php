...
class UserlogController extends Controller
{
    ...
    public function transLog()
    {
      ...
      $trans = DB::table('translogs')->join('users','users.id','=','translogs.user_id')->orderBy('translogs.id','desc')->paginate(10);
        $sent_count = Translog::where('type', '0')->count();
        $sent_amount = Translog::where('type', '0')->sum('amount');
              $rec_count = Translog::where('type', '1')->count();
                $rec_amount = Translog::where('type', '1')->sum('amount');
       
        return view('admin.user.translog', compact('trans','sent_count','sent_amount','rec_count','rec_amount'));
    }
    ...
    public function lock($id)
    {
      ...
      return redirect()->back()->with('success','User has been Locked');
    }
    ...
    public function unlock($id)
    {
      ...
      return redirect()->back()->with('success','User has been Unlocked');
    }
    ...
    public function banusers()
    {
      ...
      public function banusers_borrow()
    {
        $users = User::where('status', '0')->where('user_type',2)->orderBy('id', 'desc')->paginate(10);
        return view('admin.borrow.banned', compact('users'));
    }
